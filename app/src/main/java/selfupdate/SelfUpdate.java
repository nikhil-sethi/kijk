...
import com.example.kijk.BuildConfig;
import com.example.kijk.R;
import selfupdate.github.Release;
import selfupdate.github.Asset;
...
public class SelfUpdate {
    ...
    private static boolean isSemverGreater(String a, String b){
        // split strings into major, minor, patch
        String regex = "[.]";
        String[] aArr = a.split(regex);
        String[] bArr = b.split(regex);
        Log.d("version release: ", Arrays.toString(aArr));
        Log.d("version current: ", Arrays.toString(bArr));

        // compare ints
        boolean minorGreater = Integer.parseInt(aArr[1])>Integer.parseInt(bArr[1]);
        boolean patchGreater = Integer.parseInt(aArr[2])>Integer.parseInt(bArr[2]);
        boolean majorGreater = Integer.parseInt(aArr[0])>Integer.parseInt(bArr[0]);

        return  (majorGreater || minorGreater) || patchGreater;
    }
    public static void checkUpdate(AppCompatActivity activity, String username, String repository) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (release != null) {
                    try {
                        if (isSemverGreater(release.getTag_name(), BuildConfig.VERSION_NAME))
